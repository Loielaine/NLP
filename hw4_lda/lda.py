...
class LdaTopicCounts:
	...
	def report(self, vocab, handle, limit = 50):
		"""
		...
		for kk in self._normalizer:
			...
			word = 0
			# Sort words of each topic by prob
			porb_list = [self.word_in_topic(kk, ww) for ww in self._topic_term[kk]]
			sorted_ww = [ww for prob, ww in sorted(zip(porb_list, self._topic_term[kk]), reverse = True)]
			for ww in sorted_ww:
...
class Sampler:
	...
	def sample_probs(self, doc_id, index):
		...
		for kk in range(self._num_topics):
			...
			# uniform probability.
			# conditional prob of term given topic
			term_topic = self._topics.word_in_topic(kk, term)
			# print(term_topic)
			# print('sum(self._doc_counts[%d].values(): ' % doc_id, sum(self._doc_counts[doc_id].values()))

			# conditional prob of topic given doc
			topic_doc = float(self._doc_counts[doc_id][kk] + self._alpha[kk]) / \
			            float(sum(self._doc_counts[doc_id].values()) + sum(self._alpha))
			sample_probs[kk] = term_topic * topic_doc
		return sample_probs
	...
	def sample_doc(self, doc_id, debug = False):
		...
		for index in range(len(one_doc_topics)):
			...
			# old probs given current assignment
			...
			if self._rand_stub:
				...
				cutoff = random()
			# assign new topic given samle_probs
...
def tokenize_file(filename):
	...
	for ii in kTOKENIZER.tokenize(contents):
		...
		# should use yield when we want to iterate over a sequence, but don’t want to store the entire
		# sequence in memory.
		yield ii
...
if __name__ == "__main__":
	...
	argparser.add_argument("--doc_dir", help = "Where we read the source documents",
	                       type = str, default = "./data/wiki/", required = False)
	...
	argparser.add_argument("--output", help = "Where we write results",
	                       type = str, default = "./output/wiki", required = False)
	...
	# print(lda._doc_counts)
	# print(sum(lda._doc_counts[0].values()))
