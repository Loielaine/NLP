...
def train(save_dir = 'saved_weights',
	...
	for epoch in range(1, num_epochs + 1):
		...
		for i, (train_x, train_y) in enumerate(get_minibatches([dataset.train_inputs,
			...
			pos_inputs_batch = torch.from_numpy(np.array(pos_inputs_batch)).to(device)  # TODO
			...
			# function in numpy.
			labels = np.argmax(train_y, axis = 1)  # TODO
			...
			if i == 0 and epoch == 1:
				print("size of word inputs: ", word_inputs_batch.size())  # torch.Size([2048, 18])
				print("size of pos inputs: ", pos_inputs_batch.size())  # torch.Size([2048, 18])
				print("size of dep inputs: ", dep_inputs_batch.size())  # torch.Size([2048, 12])
				print("size of labels: ", labels.size())  # torch.Size([2048])
			...
			if i % print_every_iters == 0:
				...
				acc = int((outputs.argmax(1) == labels).sum()) / len(labels)
				acc_list.append(acc)
				print('Epoch: %d [%d], loss: %1.3f, acc: %1.3f' \
				      % (epoch, i, loss.item(), acc))
		print("End of epoch")
	...
	plt.ylabel('accuracy')
	plt.title('Accuracy of training data for 5 epoches')
	plt.grid(True)
	fig1.savefig("figure/acc.png")

	fig2 = plt.figure()
	plt.plot(loss_list)
	plt.xlabel('iteration')
	plt.ylabel('loss')
	plt.title('Loss of training data for 5 epoches')
	plt.grid(True)
	fig2.savefig("figure/loss.png")

	fig3 = plt.figure()
	plt.plot(UAS_list)
	plt.xlabel('epoches')
	plt.ylabel('UAS')
	plt.title('Loss of valid data for 5 epoches')
	plt.grid(True)
	fig3.savefig("figure/UAS.png")
...
if __name__ == '__main__':
	os.chdir("/Users/Loielaine/Desktop/umich-2020/SI630/hw/hw3")
	...
	argparser.add_argument(
		...
		type = str,
		required = False)
	# args = argparser.parse_args(['--test','--load_model_file=saved_weights/parser-epoch-5.mdl'])
	...
	if args.test:
		if parser is None or args.load_model_file is not None:
			...
			if load_file is None:
				...
				load_file = 'saved_weights/parser-epoch-5.mdl'
