...
def trainer(curW1 = None, curW2 = None):
	global uniqueWords, wordcodes, fullsequence, vocab_size, hidden_size, np_randcounter, randcounter, target_words
	vocab_size = len(uniqueWords)  # ... unique characters
	hidden_size = 100  # ... number of hidden neurons
	context_window = [-4,-3,-2, -1, 1,
	                  2,3,4]  # ... specifies which context indices are output. Indices relative to target word. Don't include index 0 itself.
	...
	for j in range(0, epochs):
		...
		for i in range(start_point, end_point):
			...
			if center_token not in target_words:
				continue
...
def load_model():
	handle = open("/home/liyixi/hw2/saved_W1_target_cluster.data", "rb")
	W1 = np.load(handle)
	handle.close()
	handle = open("/home/liyixi/hw2/saved_W2_target_cluster.data", "rb")
	W2 = np.load(handle)
	handle.close()
	return [W1, W2]
...
def save_model(W1, W2):
	handle = open("/home/liyixi/hw2/saved_W1_target_cluster3.data", "wb+")
	np.save(handle, W1, allow_pickle = False)
	handle.close()

	handle = open("/home/liyixi/hw2/saved_W2_target_cluster3.data", "wb+")
...
if __name__ == '__main__':
	if len(sys.argv) == 1:
		...
		testfilename = '/home/liyixi/hw2/intrinsic-test.csv'
		words = []
		with open(testfilename) as test:
			line = test.readline()
			while True:
